...
{
	if ((s->size_a > 1) && (s->size_b > 1) && (s->a[0] > s->a[1])
		&& (s->b[0] < s->b[1]) && (!(is_sorted(s, s->a, s->size_a))
			&& (!(rev_is_sorted(s->b, s->size_b)))))
		rev_rotate_r(s, TRUE);
	else if ((s->size_a > 1) && (s->a[0] < s->a[1])
		&& (!(is_sorted(s, s->a, s->size_a))))
		rev_rotate_a(s, TRUE);
	else if ((s->size_b > 1) && (s->b[0] > s->b[1])
		&& (!(rev_is_sorted(s->b, s->size_b))))
		rev_rotate_b(s, TRUE);
}
...
void	ft_smallsort(t_stack *stack)
{
	...
	while (!(is_sorted(stack, stack->a, stack->size_a))
		|| (stack->size_b > 1 && !(rev_is_sorted(stack->b, stack->size_b))))
	{
		swap_ab(stack);
		rotate_ab(stack);
	}
